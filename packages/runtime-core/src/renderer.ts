...
export function createRenderer(options) {
  const {
    ...
    patchProp: hostPatchProp, //patchProp方法是Vue3渲染器的一个核心方法，它负责将组件状态的变化更新到DOM元素上，从而实现视图的响应式更新。
    insert: hostInsert,
    remove: hostRemove,
    setText: hostSetText,
  ...
  /*
  * 当编译器处理到元素节点时，它会调用processElement方法来生成该节点的渲染函数代码。在这个过程中，processElement会做以下事情：
    生成该节点的属性的代码。
    根据属性生成合适的指令代码。
    根据节点的tag是否是"slot"，生成合适的插槽代码。
    根据节点是否有v-if、v-for等指令，生成相应的条件和循环代码。
    递归处理该节点的子节点。
  * */
  ...
  function mountComponent(initialVNode, container, parentComponent) {
    ...
    /*具体来说，instance对象中包含了组件实例的相关信息，例如：
    instance.type：组件的定义对象，包括组件名称、模板、组件选项等。
    instance.parent：组件实例的父组件实例。
    instance.vnode：组件实例对应的VNode对象。
    instance.proxy：组件实例的代理对象，用于访问组件实例的属性和方法。
    instance.attrs：组件实例的属性对象，包括组件在模板中绑定的所有属性。
    instance.slots：组件实例的插槽对象，包括组件在模板中插槽的所有内容。
    instance.emit：用于触发组件实例的自定义事件。*/
